...
def construct_waltz():
    minuet = []
    for m in range(0,len(minuet_table)):
        num = roll_dice(2)
        mStr = "" + minuet_table[m][num]
        minuet.append(mStr)
    trio = []
    for t in range(0, len(trio_table)):
        num = roll_dice()
        tStr = "" + trio_table[t][num]
        trio.append(tStr)

    objects = []
    for mo in minuet:
        objects.append(simpleaudio.WaveObject.from_wave_file("mozart/"+ minuet_filename(mo)))
    for to in trio:
        objects.append(simpleaudio.WaveObject.from_wave_file("mozart/"+ trio_filename(to)))
